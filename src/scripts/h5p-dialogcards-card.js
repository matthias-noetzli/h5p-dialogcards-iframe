...
class Card {
  ...
  createCardContent(card) {
    ...
    if (card.questionVideo) {
      // Add video iframe
      this.$questionVideo = $(
        `<iframe width="100%" src='https://tube.switch.ch/embed/${card.questionVideo}' frameborder='0' allow='fullscreen' class='h5p-dialogcards-card-answer-video' style='aspect-ratio:16/9;'>`
      ).appendTo($cardTextInnerContent);
    }
    ...
    if (card.answerVideo) {
      // Add video iframe
      this.$answerVideo = $(
        `<iframe width="100%" src='https://tube.switch.ch/embed/${card.answerVideo}' frameborder='0' allow='fullscreen' class='h5p-dialogcards-card-answer-video hide' style='aspect-ratio:16/9;'>`
      ).appendTo($cardTextInnerContent);
    }
  ...
  turnCard() {
    ...
    setTimeout(() => {
      ...
      if (this.hasQuestionVideo()) {
        if (turned) {
          this.$questionVideo?.removeClass("hide");
        } else {
          this.$questionVideo?.addClass("hide");
        }
      }

      if (this.hasAnswerVideo()) {
        if (turned) {
          this.$answerVideo?.addClass("hide");
        } else {
          this.$answerVideo?.removeClass("hide");
        }
      }
   ...
   */
  hasQuestionVideo() {
    return this.card.questionVideo ? true : false;
  }
   ...
   */
  hasAnswerVideo() {
    return this.card.answerVideo ? true : false;
  }
